...
local nativeFuncs = {
    ...
    , find = { nargs = 2, impl = function( argv ) local st = tostring(argv[1]) local p = tostring(argv[2]) return string.find(st, p) end }
...
local function scan_aref( expr, index, name )
    _M.debug("scan_aref from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local args = {}
    local parenLevel = 1
    local ch
    ...
    while ( true ) do
        ...
        ch = string.sub(expr, index, index)
        if (ch == ']') then
            _M.debug("scan_aref: Found a closing bracket, subexp=" .. subexp)
            args = _comp(subexp)
            _M.debug("scan_aref returning, array is " .. name)
...
local function coerce(val, typ)
    ...
    _M.debug("coerce: attempt (" .. vt .. ")" .. tostring(val) .. " to (" .. typ .. ")")
...
local function _run( ce, ctx, stack )
    ...
    while (index <= len) do
        e = ce[index]
        _M.debug("_run: next element is " .. _M.dump(e))
        ...
        elseif (e.type == BINOP) then
            ...
            _M.debug("_run: operands are (" .. base.type(v1) .. ")" .. tostring(v1) .. ", (" .. base.type(v2) .. ")" .. tostring(v2))
            if (e.op == '.') then
                _M.debug("_run: descend to " .. tostring(v2))
        ...
        elseif (e.type == UNOP) then
            ...
            elseif e.op == '#' then
                _M.debug("_run: # unop on " .. tostring(v))
                local vt = base.type(v)
                if vt == "string" then
                    v = #v
        ...
        elseif (e.type == FREF) then
            ...
            if nativeFuncs[e.name] ~= nil then
                _M.debug("_run: func=" .. _M.dump(nativeFuncs[e.name]))
            ...
            if (impl == nil and ctx['__functions'] ~= nil) then
                ...
                _M.debug("_run: context __functions provides implementation")
            end
            if impl == nil then
                _M.debug("_run: context provides deprecated-style implementation")
                impl = ctx[e.name]
            end
        ...
        elseif (e.type == VREF) then
            ...
            if index < len then
                ...
                if base.type(lookahead) == 'table' and lookahead.type == BINOP and lookahead.op == '.' then
                    isLook = true
                    v = table.remove(stack)
                    _M.debug("_run: Lookahead found subref through " .. _M.dump(v))
            ...
            if (e.index ~= nil) then
                ...
                _M.debug("_run: vref " .. e.name .. " applying subscript " .. tostring(ix))
                if ix ~= nil then
                    v = v[ix]
        ...
        _M.debug("_run: pushing result to stack: (" .. base.type(v) .. ")" .. tostring(v))
